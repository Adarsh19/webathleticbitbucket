...
class ProductsController extends Controller
{
    ...
    public  function  productogroups(Request $request,$companyid){


        //$user = Auth::user();
        $productgroups = ProductGroups::where(['parent_id' => $companyid])->where('group_type','products')->orderBy('created_at', 'desc')->paginate();
        ...
        if(empty($user)){
            ...
            $products = ProductMedias::select('*', DB::raw('count(*) as total'))->where('user_id', $companyid)
                ->whereNull('group_id')
                ->orderBy('created_at', 'desc')->groupby('category')->get();
            $variationso = ProductMedias::whereNull('group_id')->where('user_id',$companyid)->orderBy('created_at', 'desc')->get();
        ...
        $companies=Company::select('user_id','company_name')
            ...
            ->get();
        return view('admin.products.index', compact('user','productgroups','variations','products','orders','user','companies'));
    }
    ...
    public  function viewOrders(Request $request,$companyid){
        ...
        $orders="";
            ...
            $query=UserOrders::select('user_orders.*','users.name',DB::raw('count(*) as quantity'))
                ->where('status','paid')
                ->where('user_orders.userid',$companyid)
    ...
    public  function searchOrders(Request $request,$companyid){
...
//        dd($orders->toArray());
    ...
    public function loaduserlastaccessed(Request $request,$companyid){
    ...
    public function addProductGroup(Request $request,$companyid){
        ...
        $is_presant=ProductGroups::where('slug',$slug)
        ...
        if(empty($is_presant)) {
            ...
            Notify::success("Product Group ".$request->groupname." Created Successfully");
//            Session::flash('alert-success', "Product Group ".$request->groupname." Created Successfully");
        }
        else{
            Notify::info("Product Group  ".$request->groupname." already exists");
//            Session::flash('alert-warning', "Product Group  ".$request->groupname." already exists");
    ...
    public function groups($exerciseid,$groupslug){
       ...
       // $user = Auth::user();
        ...
        if(!empty($group)){
            ...
            $user = User::where('id',$exerciseid)->first();
            if(isset($user)){
            ...
            $companies=Company::select('user_id','company_name')
                ->where('parent_id',$exerciseid)
                ->get();
            return view('admin.products.index', compact('productgroups','group','products','variations','user','companies'));
        }
        else{
            abort(404);
        }
    ...
    public function addProduct(Request $request,$companyid){
        //$products=array();
        //$user = Auth::user();
        ...
        if($_FILES['imagefile']['size'][0]==0){
            ...
            Notify::error("Please choose image for the product");
            return redirect()->back();
        }
...
//        }
    ...
    public function subgroupproducts($companyid,$subgroupslug){
        //$user = Auth::user();
...
//            $user = Auth::user();
            ...
            $companies=Company::select('user_id','company_name')
                ...
                ->get();
            return view('admin.products.index', compact('variations','group','subgroup','products','user','companies'));
        }else{
...
//            Session::flash('alert-success', "Product delete successfully");
            ...
            Notify::error("Error occurred while deleting");
//            Session::flash('alert-warning', "Error occurred while deleting");
    ...
    public function deleteproductssubgroup($gid){
        ...
        if (count($story)) {
            ...
            Notify::success("Product Sub Group delete successfully");
            return redirect()->back();
        } else {
            Notify::error( "Sub Group not found");
        }
        return redirect()->back();
    }
    ...
    public function createorder($userid,$productid){
        ...
        if($product->unlimited_stock==0 && $product->stock==0){
            ...
            $message="No Stock available for the product, Click Edit  to update stock and than add again";
            ...
            try{
                ...
                if(!empty($userorder)){
                    ...
                    foreach($productobj->toArray() as $productob) {
                        ...
                        $tax_amount=($productob['price'] * ($productob['tax'] * 0.01));

                        $amount = $productob['price'] * $savedqty;
...
//        exit;
    ...
    public  function  savecart(Request $request){
        ...
        Notify::success('Cart Successfully Saved');
        return redirect()->back();
...
//        $orderid=$request->orderid;
        ...
        $orderid=$update->first()->orderid;
        $uparr = ['price' => $request->price,'quantity'=>$request->quantity,'name'=>$request->name];
        $update->update($uparr);
...
//        ]);
        ...
        $productobj=UserHasProducts::where('orderid',$orderid)->get();

        $invoiceamount=0;
        foreach($productobj->toArray() as $productob) {
            $amount = $productob['price'] * $productob['quantity'];
            $invoiceamount+=$amount;
        }
        $res=UserOrders::where('id',$orderid)->update([
            'invoiceamount' => $invoiceamount
        ...
        if($update){
            ...
            Notify::error('Some error occurred while updating order details');
        }
        return redirect()->back();
    }
    ...
    public function deleteCartItem(Request $request,$pid=null){
        ...
        $userid=$request->userid;
        $orderid=$request->orderid;
        $query=UserHasProducts::where('productid',$pid)
            ->where('orderid',$orderid)
            ->where('userid',$userid);
        ...
        if(!empty($getrow)){
            ...
            $amount_to_delete=$getrow->quantity * $getrow->price;
        }

        $delete=$query->delete();
        if($delete){

            UserOrders::where('id',$userid)->update([]);

            $orders = UserOrders::find($orderid);
            $amount = $orders->invoiceamount - $amount_to_delete;
            $orders->update(['invoiceamount' => $amount]);


            Notify::success('Order item deleted successfully');
        }else{
            Notify::error('Some error occurred while deleting order item');
        }
        return redirect()->back();
    }
    ...
    public function savetransaction(Request $request){
        ...
        Notify::success('Transactions for order  #'.$orderid.' Successfully Saved');
        return redirect()->back();
    }
    ...
    public function cancelorder(Request $request){
        ...
        ('userid_products');
        Session::save();
        Notify::success("Order #".$orderid." Successfully Delete");
        return redirect()->back();
    }
    ...
    public function  editprdouctgroup(Request $request,$id){
        ...
        if(empty(ProductGroups::where('name',$name)->first())){
            ...
            if($edit)
                ...
                Notify::error("Error occurred while updating");
        }else{
    ...
    public function editproducts(Request $request,$pid){
        ...
        if(!isset( $image)){
            ...
            $destinationPath = public_path('/admin/images/groups/products/');
            $r=$image->move($destinationPath, $name);

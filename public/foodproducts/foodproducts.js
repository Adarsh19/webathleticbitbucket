...
function createSchedule(productid){
    ...
    var selectedtopgroup=parseInt($(".nav-tabs .active > a").attr("href").replace("#tab",""));
    var exp=productid.split("_");
    var pid=exp[1];
    $('.grouptop'+pid).val(selectedtopgroup);
    $('#addProductToGroup'+pid).modal('show');
    ...
    //     userids+=$(this).val()+",";
    // });
    var url=BASE_URL+"/admin/food-products/assign-meal/"+productid+"/"+schedule_id;
...
//     var url=BASE_URL+"/admin/food-products/load-drop-area/"+companyid+"/"+scheduleid;
//
...
function clickoptionUserExercises(ds,company_id){
    ...
    if($("#userrow_"+id).length==0){
        ...
        var url=BASE_URL+"/admin/food-products/addUserToSchema/"+id+"/"+schemaid;
        $.ajax({
            url: url,
            method: "GET",
            dataType: "json",
...
function removeUser(ds,company_id){
    ...
    var url=BASE_URL+"/admin/food-products/deleteUserToSchema/"+userid+"/"+schemaid;
    $.ajax({
        url: url,
        method: "DELETE",
        dataType: "json",
...
function showhideIcons(ds) {

    $(ds).find('.icons').toggle();
...
function loadlist(companyid){
    ...
    if($('#scheduleid').val().length>0){
        ...
        var url=BASE_URL+"/admin/exercises/show-schedules/"+userids+"/"+companyid+"/"+scheduleids;
        $.ajax({
            url: url,
            method: "GET",
            dataType: "html",
...
function addUnitRow(){
    ...
    var leng=$('.unitstbody').find('tr').length;
    var val=leng+1;
    var html='<tr><td><input class="form-control" type="text" name="foodunit[]" value=""></td><td><input class="form-control" type="text" style="float:left; width: 100px; margin-right: 5px;" name="amounts[]"';
    html+='value=""><select class="form-control" name="foodweights[]" style="height: 33px;width: 100px"><option value="gramml">gram / ml</option>';
    html+='  <option value="gram" selected="selected">gram</option>';
    html+='  <option value="ml">ml</option>';
    html+='   </select></td>';
    html+='  <td><input class="form-control"  type="radio" name="sel_default" checked="checked" value="'+val+'"  style="margin-top: 3px; margin-left: 5px;">';
    html+='   <label id="default_label" style="margin-left: 5px;">Standardunit';
    html+='   </label>';
    html+='   </td>';
    html+='   </tr>';
    $('.unitstbody').append(html);
}
//
//
//
...
function show_options(ds){

    $('#sidepopup').modal('show');

}

function show_delete_popup(ds,date) {
    ...
    $('.checks').each(function (index) {
        ...
        if($(this).prop('checked')){
            ...
            if(typeof($(this).attr('meal_id')) !== 'undefined'){
                ids+=$(this).attr('meal_id')+",";
            }
        }
    });
...
$(document).ready(function(){
    ...
    var user_id=$('#user-chart-id').val();
    $('.datttpicker').datepicker({
        format: 'dd-mm-yyyy',
        autoclose: 'true',
        todayBtn: 'true',
        todayHighlight: 'true',
        ...
        window.location.href = BASE_URL+"/admin/food-products/nutrition/training/"+user_id+"/"+ds.format();
    });

...
@section('style')
    ...
    <link rel="stylesheet" href="{{ asset('/admin_files/vendor/fullcalendar/fullcalendar.min.css') }}">
    <link rel="stylesheet" href="{{ asset('/admin_files/vendor/fullcalendar/fullcalendar.print.css') }}" media="print">
    <link rel="stylesheet" href="{{ asset('/admin_files/vendor/bootstrap-colorpicker/css/bootstrap-colorpicker.min.css') }}">
...
@section('content')
            ...
            <div class="col-lg-12">
                ...
                <div class="tabs">
                    <ul class="nav nav-tabs" id="calendarTab">
                        ...
                        @foreach($service_schedules as $key=>$service_schedule)
                        <li class="nav-item @if($key==0) active @endif">
                            <a id="tab{{$service_schedule->id}}" style="border-top-color: {{$service_schedule->color}};" class="nav-link schtabs"  caltabid="{{$service_schedule->id}}" href="#tab_{{$service_schedule->id}}" data-toggle="tab">{{ $service_schedule->title }}

                                &nbsp;
                                <div class="tools pull-right editbuttons">
                                    <i class="fa fa-trash-o pointer" data-toggle="modal" data-target="#delete-schedule_modal{{$service_schedule->id}}"></i>
                                    <i class="fa fa-edit pointer" data-toggle="modal" data-target="#edit-schedule_modal{{$service_schedule->id}}"></i>
                                </div>
                            </a>

                        </li>

                            <div id="delete-schedule_modal{{$service_schedule->id }}" class="modal modal-danger fade">
                            ...
                            <div class="modal fade" id="edit-schedule_modal{{$service_schedule->id}}" tabindex="-2" role="dialog" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <form method="post"  action="{{ route('admin.editScheduleTabRqst',$service_schedule->id) }}">

                                            {{csrf_field()}}

                                            <div class="modal-header">

                                                <h4 class="modal-title modtil">Edit Schedule {{$service_schedule->title}}</h4>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                            ...
                                                            <div class="col-md-12">
                                                                ...
                                                                <div class="form-group">
                                                                    ...
                                                                    <div class="input-group">
                                                                        <input value="{{$service_schedule->title}}" type="text" id="schedule_title" name="schedule_title" value="" class="form-control" placeholder="Schedule Title">
                                                                ...
                                                                <div class="form-group">
                                                                    ...
                                                                    <select class="form-control mb-3" name="role">
                                                                        @foreach( $roles as $role)
                                                                            <option @if($service_schedule->role==$role->id) selected @endif value="{{ $role->id }}">{{$role->role}}</option>
                                                                        @endforeach
                                                                    </select>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div class="col-sm-12">
                                                                ...
                                                                <div class="book">
                                                                    ...
                                                                    <div class="checkbox-custom checkbox-primary">
                                                                        <input @if($service_schedule->dragdrop) checked @endif  name="drag" type="checkbox" id="drag">
                            ...
                            {{--<div class="form-group row pull-right">--}}
                                ...
                                {{--@if($user->role=="admin")--}}
                                    {{--<div class="col-lg-12">--}}
                                        ...
                                        {{--<select lass="form-control  companyfilter" onchange="location = this.value;">--}}
                                            {{--@foreach($companies as $company)--}}
                                                {{--<option @if(Request::segment(3)==$company->user_id) selected @endif value="{{route('admin.roosterRqst',$company->user_id)}}">{{$company->name}}</option>--}}
                                            {{--@endforeach--}}
                    ...
                    <div class="tab-content">
                        @foreach($service_schedules as $key=>$service_schedule)
                        <div id="tab_{{$service_schedule->id}}" class="tab-pane @if($key==0) active @endif">
                            <p>{{ $service_schedule->title }}</p>
                            ...
                            <div class="row calendercontent" data-loading-overlay data-loading-overlay-options='{ "startShowing": false }' style="min-height: 150px;">
                               {{--contenst from calendar will be loaded here--}}
                                ...
                                <section class="card col-lg-9 pull-left">
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-lg-12">
                                                <div id="calendar{{$service_schedule->id}}">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg-12">
                                                ...
                                                <div class="box-body no-padding">
                                                    <div id="calendarsmall{{$service_schedule->id}}" style="width: 100%"></div>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="tabs">
                                                <ul class="nav nav-tabs nav-justified">
                                                    <li class="nav-item active">
                                                        <a class="nav-link active" href="#user_tab{{$service_schedule->id}}" data-toggle="tab">Leden</a>
                                                    </li>
                                                    <li class="nav-item">
                                                        <a class="nav-link" href="#service_tab{{$service_schedule->id}}" data-toggle="tab">Diensten</a>
                                                    </li>
                                                </ul>
                                                <div class="tab-content">
                                                    <input type="hidden" class="hdden_schedule_id" value="{{$service_schedule->id}}">
                                                    <div id="user_tab{{$service_schedule->id}}" class="tab-pane active">
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            <div class="col-lg-12">
                                                                ...
                                                                <form method="post" action="{{route('admin.saveConnectedUserRqst')}}" id="formsaveconnecteduser">
                                                                    {{csrf_field()}}
                                                                    ...
                                                                    <div class="row">
                                                                        <div class="form-group" style="margin: 0px 10px 0px 10px;">
                                                                            ...
                                                                            <input class="schedule_id" name="schedule_id" type="hidden" value="{{$service_schedule->id}}">
                                                    ...
                                                    <div id="service_tab{{$service_schedule->id}}" class="tab-pane">

                                                        <div id='external-events'>

                                                            @foreach($services as $service)
                                                                <div user_id="service_{{$service->id}}" class="external-event badge badge-success ui-draggable ui-draggable-handle width100" data-event-class="fc-event-default"
                                                                     style="border-color: {{$service->bg_color}};background-color: {{$service->bg_color}}">{{$service->service}}
        ...
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                <form method="post"  action="{{ route('admin.addServiceScheduleRqst') }}">
                    ...
                    <input type="hidden" name="user_id" value="{{$segment=Request::segment(3)}}">
    ...
    <script type="text/javascript" src="{{asset('admin_files/vendor/moment/moment.js')}}"></script>
    <script src="https://code.jquery.com/ui/1.11.4/jquery-ui.min.js"></script>

    <script type="text/javascript" src="{{asset('/admin_files/vendor/fullcalendar/fullcalendar.min.js')}}"></script>
    <script type="text/javascript" src="{{asset('/admin_files/vendor/fullcalendar/locale/nl.js')}}"></script>

{{--    <script type="text/javascript" src="{{asset('/admin_files/vendor/bootstrap-timepicker/bootstrap-timepicker.js')}}"></script>--}}
    <script type="text/javascript" src="{{asset('/admin_files/vendor/bootstrap-colorpicker/js/bootstrap-colorpicker.min.js')}}"></script>
...
//            ini_events($('#external-events div.external-event'));
            ...
            var firts_loaded_schedule_id='{{$default_first_calendar}}';
//            console.log(firts_loaded_schedule_id);
            ...
            var url_segment='{{$segment=Request::segment(4)}}';
            var url_user='{{$segment=Request::segment(3)}}';
            url_segment_date='{{$segment=Request::segment(5)}}';
...
//            });
        ...
        function clickDeleteonPopup(){
            $('#schedule_form').attr('action','{{route('admin.deleteServiceScheduleRqst')}}');

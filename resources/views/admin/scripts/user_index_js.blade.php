...
var select_html= '<button class="btn btn-info btn-md hvr-grow-shadow add-user-form pull-right" href="#add-user-modal" data-toggle="modal"><i class="fa fa-user"></i> Add User</button>';
	select_html+= '<a class="btn btn_stattus btn-force btn-warning btn-md hvr-grow-shadow pull-right" href="{{route('admin.user_status.index')}}"><i class="fa fa-magic"></i>Status</a>';
	...
	var datatableInit = function(type,typeOriginal,objArg) {
		...
		columnRenders['history'] = [
			            ...
			            {'data':'log_name', "title":' @lang('common.log_name') ' },
			             {'data':'description', "title":' @lang('common.description') ' },
			             {'data':'causer_name', "title":' @lang('common.causer_name') ' },
			             {"render": function ( data, type, user) {
			             	return moment(user.created_at).format("MM-DD-YYYY HH:mm");
			             }
			             	,"title":' @lang('common.date_time') ' },
			             { "render": function ( data, type, user) {
		...
		var table = $table.dataTable({
		    ...
		    bDestroy: true,
			ajax: "{{ url('admin/users?type=') }}"+type+urlQuery,
			dom: 'Bfrtip',
			buttons: [
			{
